...
if (isset($_POST['submit'])) {
    if ($name != '' && $post != '' && $mobile != '' && $image != '') {
        $query = "INSERT INTO `maneging` (`name`,`post`,`mobile`,`image`) VALUES ('$name', '$post','$mobile','$image')";
...
if (isset($_POST['update'])) {
    ...
    if ($new_name != '' && $new_post != '' && $new_mobile != '') {
        $query = "UPDATE `maneging` SET `name`='$new_name',`post`='$new_post',`mobile`='$new_mobile',`image`='$old_image' WHERE `id`='$id'";
        mysqli_query($connection, $query);
    }
    if (!empty($new_image) &&$new_image !== '') {
        $query = "UPDATE `maneging` SET `image` = '$new_image' WHERE `id`='$id'";
        mysqli_query($connection, $query);
...
if(!empty($delete) && $delete !== ''){
    $id = $_GET['delete'];
    $teacher = select_data_from_id($connection, 'maneging', $id);
    delete_image($teacher, 'maneging', $id, 'image', '../upload/', $connection);
}
?>
...
<div class="modal fade" id="maneging">
...
<div class="top-content pb-4">
    <div class="d-flex align-items-center justify-content-between border-bottom py-2">
        <h4 class="custom-color-1">সকল পরিচালকবৃন্দ</h4>
        <button class="custom-btn custom-btn-success" data-bs-toggle="modal" data-bs-target="#maneging">নতুন যুক্ত করুন</button>
    </div>
    <ul class="nav">
        <li class="nav-item">
        ...
        <li class="nav-item">
            <a class="nav-link disabled px-1" href="">ম্যানেজিং</a>
        </li>
    </ul>
</div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="table-responsive-lg">
                <?php
                $query = "SELECT * FROM `maneging`";
                $result = mysqli_query($connection, $query);
                if (mysqli_num_rows($result) == 0) {
                ?>
                    <div class="">
                        <p class="alert alert-warning text-center">দুঃখিত কোনো উপস্থিতি নেই।</p>
                    </div>
                    <?php
                } else {
                    ?>
                        <table class="table table-striped table-bordered">
                            ...
                            <tbody class="table_body">
                                ...
                                <tr>
                                    <td>
                                        <div class="teacher_img"><img src="../upload/<?php echo $data['image']; ?>" class="img-thumbnail" alt="maneging"></div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center align-items-center">
                                            <div class="d-flex justify-content-center align-items-center">
                                                <a href="managing.php?edit=<?php echo $data['id']; ?>" class="custom-btn custom-btn-success"><i class="fa-solid fa-edit"></i></a>
                                                <a href="managing.php?delete=<?php echo $data['id']; ?>" class="custom-btn custom-btn-danger ms-2 delete"><i class="fa-solid fa-trash-alt"></i></a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
<?php
    ...
    $edit_data = select_data_from_id($connection, 'maneging', $edit);
?>
    <div class="row justify-content-center">
        <div class="col-12 col-md-6">
            ...
            <form action="managing.php" method="POST" enctype="multipart/form-data">
                ...
                <a href="managing.php" class="custom-btn custom-btn-danger mt-2">বাতিল</a>
            </form>
        </div>
    </div>
<?php } ?>
